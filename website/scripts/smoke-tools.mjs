...
try {
  ...
  await page.getByRole('heading', { name: 'Free image watermark tools', exact: true }).waitFor();
  await page.locator('.tool-hub').getByRole('link', { name: /Watermark an image/ }).waitFor();
  await page.locator('[data-primary-nav]').getByRole('link', { name: 'Tools', exact: true }).waitFor();
  ...
  assert.equal(
    await page.evaluate(() => performance.getEntriesByType('resource').some((entry) => entry.name.endsWith('.wasm'))),
    false,
    'C2PA Wasm must not load on the visible watermark page.'
  );
  await watermark.locator('[data-layout]').selectOption('tile');
  await watermark.locator('[data-position-field][hidden]').waitFor({ state: 'attached' });
  await watermark.locator('[data-status][data-state="success"]').waitFor();
  assert.equal(await watermark.locator('[data-position-field]').isHidden(), true);

  const fixture = await readFile(path.join(websiteRoot, 'public/media/playground-background.jpg'));
  const firstFile = { name: 'mountain-one.jpg', mimeType: 'image/jpeg', buffer: fixture };
  const secondFile = { name: 'mountain-two.jpg', mimeType: 'image/jpeg', buffer: fixture };
  await page.goto(`${origin}/tools/batch-watermark/`, { waitUntil: 'networkidle' });
  const batch = page.locator('[data-online-tool]');
  await batch.locator('[data-source-file]').setInputFiles([firstFile, secondFile]);
  await batch.locator('[data-run]').click();
  await batch.locator('[data-status][data-state="success"]').waitFor({ timeout: 90_000 });
  ...
  assert.equal((await batchDownload).suggestedFilename(), 'image-marker-batch.zip');

  await page.goto(`${origin}/tools/invisible-watermark/`, { waitUntil: 'networkidle' });
  const embed = page.locator('[data-online-tool]');
  await embed.locator('[data-run]').click();
  await embed.locator('[data-status][data-state="success"]').waitFor({ timeout: 90_000 });
  const tracedSource = await embed.locator('[data-preview]').getAttribute('src');
  assert.match(tracedSource ?? '', /^data:image\/png;base64,/);
  const tracedBuffer = Buffer.from(tracedSource.split(',', 2)[1], 'base64');

  await page.goto(`${origin}/tools/trace-checker/`, { waitUntil: 'networkidle' });
  const checker = page.locator('[data-online-tool]');
  await checker.locator('[data-source-file]').setInputFiles({ name: 'traced.png', mimeType: 'image/png', buffer: tracedBuffer });
  await checker.locator('[data-run]').click();
  await checker.locator('[data-status][data-state="success"]').waitFor({ timeout: 90_000 });
  await checker.getByText('asset-42', { exact: true }).waitFor();

  await page.goto(`${origin}/tools/recipe-builder/`, { waitUntil: 'networkidle' });
  ...
  assert.match((await recipe.locator('[data-preview]').getAttribute('src')) ?? '', /^blob:/);

  await page.goto(`${origin}/tools/recipient-trace-package/`, { waitUntil: 'networkidle' });
  ...
  assert.equal((await recipientDownload).suggestedFilename(), 'image-marker-recipient-traces.zip');
  await recipients.locator('[data-status][data-state="success"]').waitFor({ timeout: 90_000 });
  ...
  await lab.locator('[data-status]:not([data-state="busy"])').waitFor({ timeout: 180_000 });
  assert.equal(await lab.locator('[data-result-rows] tr').count(), 6);
  assert.match(await lab.locator('[data-result-rows] tr').first().innerText(), /✓/);

  await page.goto(`${origin}/tools/content-credentials/`, { waitUntil: 'networkidle' });
  ...
  await credentials.locator('[data-status]:not([data-state="busy"])').waitFor({ timeout: 90_000 });
  ...
  assert.deepEqual(pageErrors, [], `Tool runtime errors:\n${pageErrors.join('\n')}`);
  console.log('Verified tool hub, visible and tiled rendering, batch ZIP, invisible embed/detect, Recipe preview, recipient trace ZIP, robustness matrix, lazy C2PA inspection, and mobile layout.');
} finally {
  await browser?.close();
